...
func TestRabinKarp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := RabinKarp(tt.text, tt.pattern)

			if len(result) != len(tt.expected) {
				t.Errorf("RabinKarp() returned %d matches, expected %d", len(result), len(tt.expected))
			}

			for i, exp := range tt.expected {
				if i >= len(result) {
					t.Errorf("RabinKarp()[%d] missing, expected %d", i, exp)
				} else if result[i] != exp {
					t.Errorf("RabinKarp()[%d] = %d, expected %d", i, result[i], exp)
				}
			}
		})
	}
...
func TestRabinKarpMultiplePatterns(t *testing.T) {
	tests := []struct {
		name     string
		text     string
		patterns []string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := RabinKarpMultiplePatterns(tt.text, tt.patterns)

			if len(result) != len(tt.expected) {
				t.Errorf("RabinKarpMultiplePatterns() returned %d patterns, expected %d", len(result), len(tt.expected))
			}
